...
#
#   Usage:  .\scripts\preview.ps1 [port]     (default 5200)
#
...
# audio work everywhere. Nothing here needs the internet: no engine is
# downloaded and no file is sent.

$ErrorActionPreference = 'Stop'
Push-Location (Join-Path $PSScriptRoot '..')
try {
    $port = if ($args.Count -ge 1) { $args[0] } else { '5200' }
    ...
    Write-Host "Universal Compress -> http://localhost:$port" -ForegroundColor Green
