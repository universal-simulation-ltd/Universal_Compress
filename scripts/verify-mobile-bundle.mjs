...
//
// The fault: Universal Compress is hosted at opensource.unisim.co.uk/compress/, so the
// production build sets Vite's `base` to `/compress/` and index.html asks for
// `/compress/assets/index-….js`. Capacitor serves the bundle from `capacitor://localhost`,
// whose document root IS this copied directory — there is no `/compress/` inside it, so
